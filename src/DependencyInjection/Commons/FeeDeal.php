...
class FeeDeal{
    ...
    public function __construct($doctrine, $mongo, $balance_manipulator, $container, LoggerInterface $logger){
    ...
    public function inversedDeal(Group $creator, $amount, $service_cname, $type, $currency, $fee, $transaction_id, $version){
        ...
        if($creator->getId() != $rootGroupId){
            $this->logger->info('make transaction -> inverseddeal not admin ');
            //obtenemos el grupo
            ...
            $this->logger->info('make transaction -> inverseddeal admin ');
            $total = 0;
            $variable = 0;
            $fixed = 0;
        }
        ...
        foreach($wallets as $wallet){
            ...
            if($wallet->getCurrency() === $currency){
                $this->logger->info('make transaction -> deal admin fee');
                //Añadimos la pasta al wallet
                $wallet->setAvailable($wallet->getAvailable() - $fee + $total);
                $wallet->setBalance($wallet->getBalance() - $fee + $total);
                $em->persist($wallet);
                ...
                $dm->persist($transaction);
                $dm->flush();
                $this->logger->info('make transaction -> deal admin fee id => '.$transaction->getId());
        ...
        if($creator->getId() != $rootGroupId){
            ...
            $dm->persist($feeTransaction);
            $dm->flush();
            $this->logger->info('make transaction -> deal not admin '.$feeTransaction->getId());
