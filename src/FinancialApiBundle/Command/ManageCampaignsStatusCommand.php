...
class ManageCampaignsStatusCommand extends SynchronizedContainerAwareCommand
{
    ...
    protected function configure(){
        $this->setName('rec:manage:campaigns')
            ->setDescription('manage campaigns status. Open and close campaigns automatically');
    }
    protected function executeSynchronized(InputInterface $input, OutputInterface $output)
    {
        // TODO: Implement executeSynchronized() method.
        $em = $this->getContainer()->get('doctrine')->getManager();
        $campaigns_v2 = $em->getRepository(Campaign::class)->findBy(array('status' => [Campaign::STATUS_CREATED, Campaign::STATUS_ACTIVE], 'version' => 2));
        $now = new \DateTime();
