...
class POSIncomingController extends RestApiController{
    ...
    public function createTransaction(Request $request, $version_number,  $id){
        ...
        $service = $this->get('net.app.services.'.$service_cname.'.v'.$version_number);
        $dataIn = array();
    ...
    public function createTransactionV2(Request $request,  $id){
        ...
        $pos_config = $this->container->get('net.app.config.pos_'.strtolower($posType))->getInfo();
    ...
    public function generateAddress($type){
        if($type == 'BTC'){
            return $this->container->get('net.app.provider.btc')->getnewaddress();
        }
        elseif ($type == 'FAC'){
            return $this->container->get('net.app.provider.fac')->getnewaddress();
        }
    }
    ...
    public function checkTransaction(Request $request, $id){
        ...
        $transaction = $em->getRepository('FinancialApiBundle:Transaction')->find($id);
    ...
    public function find(Request $request, $version_number, $pos_id){
        ...
        $em = $this->getDoctrine()->getManager();
        $pos = $em->getRepository('FinancialApiBundle:POS')->findOneBy(array(
            'pos_id'  =>  $pos_id,
            'group'  =>  $userGroup
        ));
