...
class NFCController extends RestApiController{
    ...
    public function index(Request $request){
        ...
        $cards = $em->getRepository('FinancialApiBundle:NFCCard')->findAll();
...
//        if($kyc->getEmailValidated() == false) throw new HttpException(403, 'Email not validated');
        ...
        //currency FAC
        $sender_wallet = $em->getRepository('FinancialApiBundle:UserWallet')->findOneBy(array(
            'group'  =>  $company,
            'currency'  =>  Currency::$FAC
        ));

        $receiver_wallet = $em->getRepository('FinancialApiBundle:UserWallet')->findOneBy(array(
        ...
        if($request->request->has('currency_in')){
            ...
            if($currency_in != Currency::$FAC){
                //do exchange
                $amount = $this->get('net.app.commons.exchange_manipulator')->exchangeInverse($params['amount'], $currency_in, 'FAIRP');
            }
        }
    ...
    private function _sendUpdateCardEmail(NFCCard $card, $action, $url){
        ...
        $mailer = 'mailer';
        $template = 'FinancialApiBundle:Email:NFCUpdate.html.twig';
