...
class MethodsController extends BaseApiController
{
    ...
    {
        return "FinancialApiBundle:StatusMethod";
    }
    ...
    public function index() {
        ...
        $services = $this->get('net.app.method_provider')->findAll();
        ...
        if ($this->get('security.authorization_checker')->isGranted('ROLE_SUPER_ADMIN')) {
            ...
            foreach($services as $service){
                ...
                if($service->getCname() == 'btc' || $service->getCname() == 'fac'){
                    if($service->getCname() == 'fac'){
                        $cryptoWallet = $this->container->get('net.app.wallet.fullnode.fair');
                    }else{
                        $cryptoWallet = $this->container->get('net.app.wallet.fullnode.'.$service->getCname());
                    }
            ...
            foreach($services as $method){
                if(in_array($method->getCname().'-'.$method->getType(), $group_services)){
                    ...
                    $methodsEntity = $this->get('net.app.method_provider')->findByCname($method->getCname().'-'.$method->getType());
    ...
    public function indexSwift() {
        ...
        if ($this->get('security.authorization_checker')->isGranted('ROLE_SUPER_ADMIN')) {
            ...
            foreach($services as $service){
                ...
                $method_in = $this->get('net.app.in.'.$methods[0].'.v1');
                $method_out = $this->get('net.app.out.'.$methods[1].'.v1');
    ...
    public function indexExchanges() {
        ...
        if ($this->get('security.authorization_checker')->isGranted('ROLE_SUPER_ADMIN')) {
            ...
            $exchanges = $em->getRepository('FinancialApiBundle:StatusMethod')->findBy(array(
                'type'  => 'exchange'
            ));
