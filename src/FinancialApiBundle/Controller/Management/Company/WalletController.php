...
//                            array_key_exists($res->getCurrency(), $data[$day])? $data[$day][$res->getCurrency()] += $res->getAmount():$data[$day][$res->getCurrency()] = $res->getAmount();
    ...
    public function currencyExchange(Request $request, $company_id){
        ...
        $userGroup = $em->getRepository('FinancialApiBundle:Group')->find($company_id);
    ...
    public function indexFees(Request $request, $account_id){
        //get group
        $em = $this->getDoctrine()->getManager();
        $company = $em->getRepository('FinancialApiBundle:Group')->find($account_id);
        ...
        //get methods by tier
        $methods = $this->get('net.app.method_provider')->findByTier($company->getTier());
        $activeFees = [];
