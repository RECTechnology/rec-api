...
class WalletController extends RestApiController{
    ...
    public function single(Request $request, $id){
        $dm = $this->get('doctrine_mongodb')->getManager();
        $userGroup = $this->get('security.token_storage')->getToken()->getUser()->getActiveGroup();
        $last10Trans = $dm->createQueryBuilder('FinancialApiBundle:Transaction')
    ...
    public function walletTransactions(Request $request){
        ...
        if($request->query->get('query') != ''){
            ...
            $transactions = $dm->getRepository('FinancialApiBundle:Transaction')->findTransactions($userGroup, $start_time, $finish_time, $search, 'id', 'desc');
...
//                            array_key_exists($res->getCurrency(), $data[$day])? $data[$day][$res->getCurrency()] += $res->getAmount():$data[$day][$res->getCurrency()] = $res->getAmount();
    ...
    public function _getBenefits($interval, $start = null, $end =null){
        ...
        $dm = $this->get('doctrine_mongodb')->getManager();
        $result = $dm->getRepository('FinancialApiBundle:Transaction')->getBenefits($userGroup, $start_time, $end_time);
    ...
    public function getPdfReceipt(Request $request, $id){
        ...
        $html = $this->container->get('templating')->render('FinancialApiBundle:Email:receipt.html.twig', $body);
