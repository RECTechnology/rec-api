...
class PaymentOrdersController extends CRUDController {
    ...
    function updateAction(Request $request, $role, $id)
    {
        if($request->request->has('status') && $request->request->get('status') === PaymentOrder::STATUS_REFUNDED){

            $paymentOrderRepo = $this->getRepository();
            $paymentOrder = $paymentOrderRepo->find($id);
            if($paymentOrder && $paymentOrder->getStatus() === PaymentOrder::STATUS_REFUNDED){
                throw new HttpException(403, 'This payment has been already refunded');
            }
            return parent::updateAction($request, $role, $id); // TODO: Change the autogenerated stub
        }

        throw new HttpException(403, "Change not allowed");
