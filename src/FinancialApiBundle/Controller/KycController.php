...
class KycController extends BaseApiController{
    ...
    public function validateEmailCode(Request $request){
        ...
        $tierValidation = $em->getRepository('FinancialApiBundle:TierValidations')->findOneBy(array(
            'user' => $user
        ));
    ...
    private function _sendEmail($subject, $body, $to, $action){
        ...
        if($action == 'register'){
            ...
            $template = 'FinancialApiBundle:Email:registerconfirmkyc.html.twig';
        }elseif($action == 'register_kyc_holy'){
            $template = 'FinancialApiBundle:Email:registerconfirmkycholy.html.twig';
