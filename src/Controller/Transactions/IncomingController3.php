...
use App\DependencyInjection\Commons\LimitManipulator;
...
use App\EventSubscriber\Doctrine\SuccessPurchaseSubscriber;
    ...
    private function outTransactionProcess($data, ?object $user_from, $account_from, Transaction $transaction, ?bool $isRefund)
    {
        ...
        if ($payment_info['status'] === 'sent' || $payment_info['status'] === 'sending') {
            ...
            if($destination->getType() === Group::ACCOUNT_TYPE_ORGANIZATION){
                ...
                $dispatcher->dispatch($purchaseEvent, PurchaseSuccessEvent::NAME);
                ...
                $dispatcher->dispatch($transferEvent, TransferEvent::NAME);
            }
