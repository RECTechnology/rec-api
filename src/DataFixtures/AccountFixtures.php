...
class AccountFixtures extends Fixture implements DependentFixtureInterface {
    ...
    public function load(ObjectManager $orm)
    {
        ...
        $user = $orm->getRepository(User::class)
            ->findOneBy(['username' => UserFixtures::TEST_USER_CREDENTIALS['username']]);

        $worker_user = $orm->getRepository(User::class)
            ->findOneBy(['username' => UserFixtures::TEST_SECOND_USER_CREDENTIALS['username']]);

        $third_user = $orm->getRepository(User::class)
            ->findOneBy(['username' => UserFixtures::TEST_THIRD_USER_CREDENTIALS['username']]);

        $user_in_shop = $orm->getRepository(User::class)
            ->findOneBy(['username' => UserFixtures::TEST_USER_IN_SHOP['username']]);
        ...
        $this->createAccount(
            ...
            self::ACCOUNT_SUBTYPE_NORMAL,
            1,
            UserFixtures::TEST_THIRD_USER_CREDENTIALS['name'],
            1000e8
        );
        $this->createAccount(
            $orm,
        ...
        $admin = $orm->getRepository(User::class)
            ->findOneBy(['username' => UserFixtures::TEST_ADMIN_CREDENTIALS['username']]);
        ...
        $user_phone_non_validated = $orm->getRepository(User::class)
            ->findOneBy(['username' => UserFixtures::TEST_USER_PHONE_NON_VALIDATED['username']]);
        $this->createAccount(
            $orm,
            $faker,
        ...
        $user_LTAB = $orm->getRepository(User::class)
            ->findOneBy(['username' => UserFixtures::TEST_USER_LTAB_CREDENTIALS['username']]);
        $this->createAccount(
            $orm,
            $faker,
            $user_LTAB,
        ...
        $user_LTAB_org = $orm->getRepository(User::class)
            ->findOneBy(['username' => UserFixtures::TEST_USER_LTAB_COMMERCE_CREDENTIALS['username']]);
        $this->createAccount(
            $orm,
            $faker,
            $user_LTAB_org,
        ...
        $manager_pos_org = $orm->getRepository(User::class)
            ->findOneBy(['username' => UserFixtures::TEST_POS_MANAGER_CREDENTIALS['username']]);
        $this->createAccount(
            $orm,
            $faker,
            $manager_pos_org,
        ...
        $rezero_org_3 = $orm->getRepository(User::class)
            ->findOneBy(['username' => UserFixtures::TEST_REZERO_USER_3_CREDENTIALS['username']]);
        $this->createAccount(
            $orm,
            $faker,
            $rezero_org_3,
    ...
    private function createAccount(ObjectManager $orm, Generator $faker, User $user, array $roles = [], string $type = self::ACCOUNT_TYPE_PRIVATE, string $subtype = self::ACCOUNT_SUBTYPE_NORMAL, int $tier = 1, string $name = null, float $balance=100e8, User $worker_user=null){
        ...
        if($name === self::TEST_ACCOUNT_REZERO_3['name']){
            ...
            $account->setRezeroB2bUsername(UserFixtures::TEST_REZERO_USER_3_CREDENTIALS['name']);
            $account->setRezeroB2bAccess(Group::ACCESS_STATE_GRANTED);
