...
class RechargeRecsTest extends AdminApiTest {
    ...
    function testRechargeCultureAndGetReward(){
        ...
        $culture_commerce_account = $this->rest('GET', "/user/v3/groups/search?name=".AccountFixtures::TEST_ACCOUNT_CULT21_COMMERCE['name']);
    ...
    {
        $no_culture_commerce_account = $this->rest('GET', "/user/v3/groups/search?name=".AccountFixtures::TEST_ACCOUNT_COMMERCE['name']);
    ...
    function testRechargeWhenAllCommerceHasKYC2AndTier0ShouldPass(): void
    {
        ...
        $em = self::createClient()->getKernel()->getContainer()->get('doctrine.orm.entity_manager');
        $user = $em->getRepository(User::class)->findOneBy(['username' => UserFixtures::TEST_USER_CREDENTIALS['username']]);
        $user_id = $user->getId();
        $user_pin = $em->getRepository(User::class)->findOneBy(['id' => $user_id])->getPin();
